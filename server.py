...
"""
2024/03/30
16:ほとんどのrpcの機能に対応しているためそれほど変更はないだろう。
19:文字数制限がわからないためすべて128文字(以下切り捨て)に制限している。また、何らかのエラーでpostの中身が殻だった場合はnoneを設定することで終了することを避けている。
36:正直あまり理解していないが(data.getの空の第二引数など)まあ機能しているからいいだろう。
61:rpcをクリアする用のhandle。多分機能しているだろう。
67:起動用。だが、本番環境で使うな！というエラーが出るのはなぜだろう、まあローカル内で完結させているので大丈夫だろう。
"""